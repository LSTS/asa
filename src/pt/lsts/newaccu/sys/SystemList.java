...
import pt.lsts.newaccu.util.IMCUtils;
	...
	public void onReceive(IMCMessage msg) {
		...
		if (msg.getAbbrev().equalsIgnoreCase("Announce")) {
			...
			if (containsSysName(msg.getString("sys_name"))) {
				...
				if (!s.isConnected()) {
					findSysByName(msg.getString("sys_name")).lastMessageReceived = System
							.currentTimeMillis();
			...
			try {
				...
				newAccu.getInstance().getIMCManager()
						.send(s.getAddress(), s.getPort(), m);
				newAccu.getInstance().getIMCManager().getComm()
